...
from dataset import WholeBrainDataset, center_crop
...
def load_models_obj(tracts,init_atlas,save_path,preTrain = True, FA_only = False):
    ...
    if not FA_only:
        for tract in backbone_tracts:
            ...
            snapshot_path = save_path.parent/f'models_sep/{tract}_trained.pt'
    ...
    # load trainable params 
    params = [p for p in model.parameters() if p.requires_grad]
    dataset = WholeBrainDataset('path/to/datasets/HCP_100', backbone_tracts, center_crop,FA_only)
    optimizer=torch.optim.Adam(params, lr=1e-3,)
    return dataset, model, optimizer
...
def main(total_epochs: int, save_every: int, batch_size: int, save_path: str, tracts: str, pretrained = False,init_atlas_file = None):
    ...
    dataset, model, optimizer = load_models_obj(tracts,save_path,init_atlas,pretrained,FA_only=False,)
