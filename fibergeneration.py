...
class HCPFiberDatasetDir(Dataset):
    def __init__(self, 
        ...
        self.backbone_tracts = ['AF', 'ATR', 'CA', 'CC',
                   ...
                   'ST_FO', 'ST_OCC', 'ST_PAR', 'ST_POSTC', 'ST_PREC', 'ST_PREF', 'ST_PREM',
                   'T_OCC', 'T_PAR', 'T_POSTC', 'T_PREC', 'T_PREF', 'T_PREM', 'UF']
        self.img_data_dir = sorted(path for path in pathlib.Path(img_data_dir).glob('*') if (path.is_dir() & ('seg' not in path.name)))[:sub_num]
        print(self.img_data_dir)
        self.transform = transform
        tmp = nib.load(list(self.img_data_dir[0].glob('**/FA_MNI*'))[0])
        self.config = (tmp.affine,tmp.header)
        self.shape = tmp.get_fdata().squeeze().shape
    ...
    def __getitem__(self, idx):
        ...
        struct_dict['FA'] = self.transform(nib.load(FA_path).get_fdata().squeeze()[::-1],indice_down,indice_up)[np.newaxis, ...]
        ...
        for tract in self.backbone_tracts:
            ...
            TOM_path = list(images_dir_path.glob(f'**/TOM_new/{tract}.nii.gz'))
            ...
            seg_path = list(images_dir_path.glob(f'**/seg_new/{tract}.nii.gz'))
...
class FiberRegistor:
    def __init__(self,
                 ...
                 model_path: str = 'snapshot100.pt',
                 data_path: str = '/data0/HCP_100',
                 device: str = 'cuda:7',
                 batch_size: int = 6,
                 sub_num: int = 10,
                 tracts_num=-1
        ...
        self.root_dir = '/results/tracts_related'
        self.device = device
        self.tracts = tracts
        self.prefixes =['FA','seg','TOM','flow','tract','vtk','WGCseg']
    ...
    def load_model(self):
        ...
        params = torch.load(self.model_path,map_location='cpu',weights_only=True)["MODEL_STATE"]
        # clean ._orig_mod
        params = {k.replace('._orig_mod',''):v for k,v in params.items()}
        self.whole_model.load_state_dict(params,assign=True)
        self.subid_list = []
    ...
    def save_results(self):
        ...
        Parallel(n_jobs=24)(delayed(nib.save)(nib.Nifti1Image(self.center_padding(flow)[...,None,:]*1.25, self.affine, flow_header),self.flowpath/f'SUB{i:03d}.nii.gz') for i,flow in enumerate(self.flows_list))
...
if __name__ =='__main__':
    ...
    arg.add_argument('--device',type=str,default='cuda:7')
    arg.add_argument('--batch_size',type=int,default=2)
    arg.add_argument('--suffix',type=str,default='alltracts_HCP')
    arg.add_argument('--model_path',type=str,default='model.pt')
    arg.add_argument('--data_path',type=str,default='/data/HCP_test')
